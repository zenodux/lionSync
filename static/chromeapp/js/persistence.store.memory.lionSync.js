...
persistence.store.memory.config = function(persistence, dbname) {
  ...
  persistence.saveToLocalStorage = function(callback) {
    ...
    persistence.loadFromChromeLocalStorage = function(callback) {
    var dump = chrome.storage.local.get(dbname, function(callback){
        if(dump) {
          this.loadFromJson(dump, callback);
        } else {
          callback && callback();
  ...
  persistence.saveToChromeLocalStorage = function(callback) {
    this.dumpToJson(function(dump) {
        chrome.storage.local.set(dbname, dump);
        if(callback) {
          callback();
        }
      });
